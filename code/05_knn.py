...
distances=[]
for i in xrange(len(iris_data)):
    distances.append((euclidean_distance(unknown,iris_array[i][0:4]), iris_array[i][4]))
...
nearest = np.sort(distances, axis=0)[0:k]
...
nearestlist= nearest.tolist()
nearestlist

nearest_flowers = [flower[1] for flower in nearestlist]
nearest_flowers
Counter(nearest_flowers).most_common(1)[0][0]
